    ...
    for t in range(2, df['signals'].size):
        if df['Close'][t] > df['20d_high'][t-1]:
            if not has_position:
                ...
                has_position = True
                content ="進場日期: "+str(df.index[t]) 
                print(content)
        elif df['Close'][t] < df['10d_low'][t-1]:
            if has_position:
                df.loc[df.index[t], 'signals'] = -1
                has_position = False
                content ="出場日期: "+str(df.index[t]) 
                print(content)
...
# data = Breakout(df).dropna()
# print(data)
# data.plot(kind = 'line', figsize=(12, 6), y=['20d_high', '10d_low'])
# plt.savefig("test.png")
# plt.show()
# ================
# 乖離率
# df = get_quotes('2330.tw', datetime(2018, 11, 1))
# data = BIAS(df).dropna()
# print(data)
# data.plot(kind = 'line', figsize=(12, 6), y=['BIAS'])
# plt.savefig("test.png")
# plt.show()