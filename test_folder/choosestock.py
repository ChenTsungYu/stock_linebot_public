...
avgprice=[]
url = 'https://www.twse.com.tw/exchangeReport/STOCK_DAY_AVG?response=json&date=' + today.strftime("%Y%m%d") + '&stockNo=' + yourstock
...
#如果不夠20日，就爬上個月的價格    
if len(avgprice) < 19:
    url = 'https://www.twse.com.tw/exchangeReport/STOCK_DAY_AVG?response=json&date=' + lastmonth.strftime("%Y%m%d") + '&stockNo=' + yourstock
    list_req = requests.get(url) #請求網站
    soup = BeautifulSoup(list_req.content, "html.parser") #將整個網站的程式碼爬下來
    jsonsoup=json.loads(str(soup))
